...
export default function Return() {
  ...
  const handleScan = async (decodedText) => {
    ...
    submitReturn(decodedText);
  };

  const submitReturn = async (isbnToReturn, userId = null) => {
    setLoading(true);
    setError('');
    
    try {
      let url = `/api/lending/return?isbn=${isbnToReturn}`;
      ...
      if (!res.ok) {
        if (res.status === 409 && data.detail?.candidates) {
          ...
          setStep(1.5); // New step for selecting candidate
          return;
        }
        ...
        <div className="space-y-6 glass-panel p-6 rounded-2xl animate-in slide-in-from-right-4">
          ...
          <div className="space-y-3 mt-4">
            {candidates.map(c => (
              <button 
                key={c.user_id}
                onClick={() => submitReturn(scannedIsbn, c.user_id)}
          ...
          <button onClick={() => setStep(1)} className="w-full py-3 mt-4 text-gray-400 text-sm font-medium hover:text-white transition-colors">
            キャンセルしてスキャンし直す
          </button>
        </div>
      )}
