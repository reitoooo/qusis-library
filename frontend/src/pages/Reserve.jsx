...
export default function Reserve() {
  ...
  const handleReserve = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.detail || '予約処理に失敗しました');
      }
  ...
  return (
    <div className="max-w-md mx-auto space-y-8 animate-in fade-in">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-gradient text-purple-400">本を予約する</h1>
      </div>
      ...
      {step === 2 && (
        <div className="space-y-6 glass-panel p-6 rounded-2xl animate-in slide-in-from-right-4">
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-4 bg-purple-500 text-white font-extrabold text-lg rounded-xl hover:bg-purple-600 hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all flex justify-center items-center gap-2 disabled:opacity-50"
          >
            {loading ? <Loader2 className="animate-spin" /> : '予約を実行する'}
          </button>
      ...
      {step === 3 && (
        <div className="text-center space-y-6 py-10 glass-panel p-6 rounded-2xl animate-in zoom-in-95">
          ...
          <div>
            <h2 className="text-3xl font-extrabold text-white mb-2">予約完了</h2>
            <p className="text-gray-400 leading-relaxed">返却され次第、Slackでお知らせします。<br />楽しみに待っていてください！</p>
          </div>
          <div className="pt-6">
